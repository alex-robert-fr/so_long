	...
	while (map[y])
	{
		...
		while (map[y][x])
		{
			if (map[y][x] == '1')
			{
				...
				if (x == 0 && map[y][x + 1] == '0')
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.sp3_0, (x * 32), (y * 32));
				}
				// Mur Droite
				else if (x == info_map.columns - 1 && map[y][x - 1] == '0')
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.sp2_0, (x * 32), (y * 32));
				}
				// Mur Haut
				else if (y == 0 && map[y + 1][x] == '0')
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.sp10_0, (x * 32), (y * 32));
				}
				// Mur Bas
				else if (y == info_map.rows - 1 && map[y - 1][x] == '0')
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.sp13_0, (x * 32), (y * 32));
				}
				// Angle 0, 0
				else if (x == 0 && y == 0)
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.sp1_0, (x * 32), (y * 32));
				}
				// Angle 0, 1
				else if (x == info_map.columns - 1 && y == 0)
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.sp0_0, (x * 32), (y * 32));
				}
				// Angle 1, 0
				else if (x == 0 && y == info_map.rows - 1)
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.sp5_0, (x * 32), (y * 32));
				}
				// Angle 1, 1
				else if (x == info_map.columns - 1 && y == info_map.rows -1)
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.sp4_0, (x * 32), (y * 32));
				}
				// Mur Haut + extension gauche
				else if (y == 0 && map[y + 1][x] == '1' && map[y + 1][x - 1] == '0')
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.sp11_2, (x * 32), (y * 32));
				}
				// Mur Haut + extension droite
				else if (y == 0 && map[y + 1][x] == '1' && map[y + 1][x + 1] == '0')
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.sp10_2, (x * 32), (y * 32));
				}
				/* -------------------------------------------------------------------------- */
				/*                                    BLOC                                    */
				/* -------------------------------------------------------------------------- */
				...
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.sp14_0, (x * 32), (y * 32));
				}
				// Bloc bas
				else if (map[y][x - 1] == '1' && map[y][x + 1] == '1' && map[y - 1][x] == '1' && map[y + 1][x] == '0')
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.sp4_1, (x * 32), (y * 32));
				}
				// Bloc gauche
				else if (map[y][x - 1] == '0' && map[y][x + 1] == '1' && map[y - 1][x] == '1' && map[y + 1][x] == '1')
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.sp9_1, (x * 32), (y * 32));
				}
				// Bloc droite
				else if (map[y][x - 1] == '1' && map[y][x + 1] == '0' && map[y - 1][x] == '1' && map[y + 1][x] == '1')
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.sp8_1, (x * 32), (y * 32));
				}
				// Angle Bloc 0, 0
				else if (map[y][x - 1] == '0' && map[y][x + 1] == '1' && map[y - 1][x] == '0' && map[y + 1][x] == '1')
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.sp7_1, (x * 32), (y * 32));
				}
				// Angle Bloc 0, 1
				else if (map[y][x - 1] == '1' && map[y][x + 1] == '0' && map[y - 1][x] == '0' && map[y + 1][x] == '1')
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.sp6_1, (x * 32), (y * 32));
				}
				// Angle Bloc 1, 0
				else if (map[y][x - 1] == '0' && map[y][x + 1] == '1' && map[y - 1][x] == '1' && map[y + 1][x] == '0')
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.sp9_2, (x * 32), (y * 32));
				}
				// Angle Bloc 1, 1
				else if (map[y][x - 1] == '1' && map[y][x + 1] == '0' && map[y - 1][x] == '1' && map[y + 1][x] == '0')
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.sp10_1, (x * 32), (y * 32));
				}
				// Interieur bloc
				else if (map[y][x - 1] == '1' && map[y][x + 1] == '1' && map[y - 1][x] == '1' && map[y + 1][x] == '1')
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.sp12_2, (x * 32), (y * 32));
				}
				else
				{
					mlx_put_image_to_window(win.mlx, win.win, imgs.spxxx, (x * 32), (y * 32));
				}
			}
			ft_putchar_fd('[', 1);
			ft_putnbr_fd(y, 1);
